...
(function ($, window, document) {
    ...
    TableTools.prototype = {
        ...
        "_fnGetDataTablesData": function (oConfig) {
            ...
            var parametro = jQuery.parseJSON(this.s.dt.oAjaxData);
            parametro.start = 0;
            parametro.length = this.s.dt._iRecordsTotal;
            parametro.exportacao=true;
            var retornoConsulta = null;

            $.ajax({
                type: 'POST',
                url: this.s.dt.ajax.url,
                async: false,
                data: parametro,
                success: function (dataFull) {
                    retornoConsulta = dataFull;
                }
            });

            for (j = 0, jLen = retornoConsulta.data.length ; j < jLen ; j++) {
                ...
                for (i = 0, iLen = dt.aoColumns.length ; i < iLen ; i++) {
                    if (aColumnsInc[i]) {
                        ...
                        var value = eval('retornoConsulta.data[' + j + '].' + dt.aoColumns[i].mData);
                        
                        if (dt.aoColumns[i].type == 'date' && value != null) {
                            var dtStart = new Date(parseInt(value.substr(6)));
                            var dtStartWrapper = moment(dtStart);
                            value = dtStartWrapper.format('DD/MM/YYYY HH:mm:ss');
                        }
                        else if (dt.aoColumns[i].type == 'bool') {
                            value = (value ? 'Sim' : unescape('N%E3o'));
                        } else if (dt.aoColumns[i].type == 'chAcesso') {
                            value = '\u200C' + value;
                        }
                        aRow.push(this._fnBoundData(value, oConfig.sFieldBoundary, regex));
                    }
                }
        ...
        "copy": $.extend({}, TableTools.buttonBase, {
            ...
            "fnComplete": function (nButton, oConfig, flash, text) {
                ...
                this.fnInfo('<h6>Tabela copiada</h6>' +
                    '<p>Copiado ' + lines + ' linha' + plural + ' para \u00e1rea de transfer\u00eancia.</p>',
                    1500
                );
            }
        }),
